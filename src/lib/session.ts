...
export async function getSessionUser(env: Env, req: Request): Promise<SessionUser | null> {
  ...
  const user = await env.DB.prepare(
    "SELECT id, name, email, role, team_id, is_captain, affiliation, country, website, banned, verified, suspended, prize_disqualified, under_review FROM users WHERE id = ?"
  )
    .bind(userId)
